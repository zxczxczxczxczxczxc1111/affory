...
func main() {
	if len(os.Args) > 1 {
		...
		case "uninstall":
			...
			// меню Пуск и запись в «Программах и компонентах» это не ключи
			// человека, а наша регистрация в системе. Оставлять её значит
			// оставлять программу числиться установленной после удаления.
			for _, zhaloba := range snyatSledy() {
...
func (s *sluzhba) Execute(args []string, r <-chan svc.ChangeRequest, st chan<- svc.Status) (bool, uint32) {
	...
	//
	// Старт это единственный момент, в который состав виден целиком и заведомо
	// не испорчен нашей же правкой. Разбор жалобы 20.09.2026 упёрся именно в его
	// отсутствие: запись подписки появлялась между двумя стартами, и сказать,
	// принесло ли её обновление или она пережила удаление, было нечем. Отметка
	// файла секретов рядом отвечает на второй вопрос: тот же это файл или новый.
	if n, err := yadro.nabor(); err != nil {
		log.Printf("подписки при старте не прочитаны: %v", err)
	} else {
		log.Printf("подписки при старте: %s; %s", opisatPodpiski(n), sledSekretov())
	}
