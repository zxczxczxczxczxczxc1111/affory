...
export function Skorost({ snapshot, error, pending, disabled, start, cancel }: {
  ...
  return (
    <section className="af-speed" aria-label="Скорость подключения">
      ...
      {running && <div className="af-speed-progress" role="status"><span className="af-speed-spinner" aria-hidden="true" />{snapshot.phase === "download" ? "Замер приёма" : "Замер отдачи"} · {snapshot.name || "Выбор сервиса"}{snapshot.attempt > 1 ? ` · попытка ${snapshot.attempt} из 4` : ""}</div>}
      ...
      <details className="af-speed-options"><summary>Сервис замера</summary>
        ...
        <p className="af-note">Если сервис недоступен, Affory попробует следующий. До минуты и примерно 550 МБ при четырёх попытках: замер меряет скорость до тестового сервиса, а не до сервера подписки</p>
