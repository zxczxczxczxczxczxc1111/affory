...
// MTU обязан стоять, и стоять именно 1500.
//
// Без него ядро поднимает интерфейс с MTU 65535, и голос под нагрузкой встаёт
// в очередь за куском видео: замер 20.09.2026 дал у trojan 59, 60, 79, 97 мс
// против 60, 60, 64, 64 на 1500. Судья стережёт само наличие поля: пропажа
// MTU не ломает ни сборку, ни подъём туннеля, она возвращает ту же задержку
// молча, и заметить это можно только повторным прогоном смеси.
//
// Число проверяется точным, а не «каким-нибудь»: 1280 здесь уже стояло
// 19.09.2026 под опровергнутую позже гипотезу, и отличить его от правильного
// значения проверка «поле не пустое» не смогла бы.
func TestMtuTunZadan(t *testing.T) {
	telo, err := SingBox(obraztsovyyVhod())
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, v := range k.Inbounds {
		...
		mtu, est := v["mtu"].(float64)
		if !est {
			t.Fatalf("у TUN нет MTU: ядро возьмёт 65535, и голос под нагрузкой встанет в очередь (поле %v)", v["mtu"])
		}
		if mtu != 1500 {
			t.Fatalf("MTU TUN %v, а замер выбрал 1500", mtu)
		}
		return
	}
	t.Fatal("в конфиге нет входящего tun")
