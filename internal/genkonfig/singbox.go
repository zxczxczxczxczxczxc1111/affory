...
func vhodyashchie(v Vhod) []any {
	vh := []any{
		map[string]any{
			...
			// MTU задан явно с 19.09.2026. Без него Windows показывал у
			// адаптера 65535, то есть приложение отправляло датаграмму любого
			// размера и узнавало о беде только потерями.
			//
			// Откуда 1280. Замер того же дня: видеопоток 8 Мбит датаграммами
			// по 1200 байт через hysteria2 терял 20-27% НА ЧИСТОЙ СЕТИ, а тот
			// же поток пакетами по 900 байт - 1.3%. Датаграмма 1200 вместе с
			// заголовками hysteria2 и QUIC не влезает в пакет, которым QUIC
			// идёт по умолчанию, и дальше её режут на части: пропала часть -
			// пропала вся датаграмма. 1280 это минимальный MTU, обязательный
			// для IPv6, то есть предел, ниже которого не опускается никто, и
			// при нём приложение шлёт не больше 1252 байт полезного.
			//
			// ВАЖНО, что этим замером правка НЕ подтверждена: прибор ходит
			// через прокси клиента, а не через TUN, и MTU адаптера его пакеты
			// не ограничивает вовсе. Ограничение ставится по устройству
			// протокола, а не по наблюдению, и ждёт проверки на живом
			// видеозвонке.
			"mtu": 1280,
		},
	}
