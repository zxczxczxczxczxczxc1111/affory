...
func vhodyashchie(v Vhod) []any {
	vh := []any{
		map[string]any{
			...
			// MTU намеренно НЕ задан. 19.09.2026 сюда на полдня попало 1280
			// под гипотезу, что hysteria2 теряет на видеопотоке из-за размера
			// датаграммы: один прогон дал 20-27% потерь на 1200 байтах против
			// 1.3% на 900. Прогон по семи размерам в три круга гипотезу
			// опроверг - потери 0-6.5% размазаны по всем размерам, а два
			// прогона дали ровный ноль как раз на 1150 и 1200 байтах.
			// Терялось не поштучно, а кусками по 50-300 пакетов подряд, и
			// такие куски есть у tuic и trojan тоже. Это провалы канала, а не
			// фрагментация. Само число к тому же было выведено из замера,
			// который TUN не задействует вовсе: прибор ходит через прокси
			// клиента. Ставить MTU по такому основанию нельзя.
		},
	}
	if v.PortProksi > 0 {
