...
const (
	...
	// Проба через сервер не уложилась в срок: ответа не было ни отказом, ни
	// успехом. Заведён 22.09.2026 (A7): до него таймаут пробы приезжал кодом
	// KodServerAuthFailed, то есть не ответивший сервер отправлял человека
	// проверять ключи и подписку, с которыми всё в порядке.
	//
	// НЕ путать с KodServerAuthFailed: там сервер ОТВЕТИЛ отказом и чинить
	// надо подписку; здесь не ответил никто, и осмысленны повтор и другой
	// сервер.
	KodProbaNeUspela = "probe-timeout"

	// Подписка не ответила в срок. Отдельно от KodSubscriptionUnreach: тот
	// отправляет обновлять подписку, а не успевший ответ лечится повтором, и
	// панель тут может быть совершенно исправна.
	KodPodpiskaSrok = "subscription-timeout"

	// Панель ответила 401, 403 или 407: живой сервер отказал по праву доступа.
	// Отдельно от KodSubscriptionUnreach ровно потому, что это единственный
	// случай, когда чинить надо именно ссылку и ключ.
	KodPodpiskaDostup = "subscription-auth-failed"
...
// A6, 22.09.2026. Новый набор правил собрался, но ядро его не приняло, и
// проверка случилась ДО остановки рабочего туннеля: правки отменены, VPN цел.
// Отдельно от rule-invalid: тот про строку, которую человек написал не так, а
// этот про набор целиком, и чинится он не правкой строки, а отказом от неё.
const KodPravilaNePrinyaty = "rules-rejected-by-core"
